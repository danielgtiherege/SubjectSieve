...
#define ENABLE_LEFT_BUTTON_DELAY nullptr
  ...
  void passthrough(std::shared_ptr<input_t> &input, std::vector<std::uint8_t> &&input_data) {
		...
		if (input_data.size() >= sizeof(NV_INPUT_HEADER)) {
			// Formato oficial: NV_INPUT_HEADER tem 'packetType' little-endian no começo
			// (estrutura definida em moonlight-common-c/Input.h)
			packetType = read_le_u16(reinterpret_cast<const uint8_t*>(input_data.data()));
		} else if (input_data.size() >= 2) {
			packetType = read_le_u16(reinterpret_cast<const uint8_t*>(input_data.data()));
		}
		metrics::log_input_packet(packetType, input_data.size());
		*/
