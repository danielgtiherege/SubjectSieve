...
namespace nvenc {
  ...
  bool nvenc_base::create_encoder(const nvenc_config &config, const video::config_t &client_config, const nvenc_colorspace_t &colorspace, NV_ENC_BUFFER_FORMAT buffer_format) {
	...
	//variaveis base de modificacao
	enc_config_snapshot = enc_config;           // copia o RC atual, profile, etc.
	init_params_snapshot = init_params;         // copia o INIT atual
	init_params_snapshot.encodeConfig = &enc_config_snapshot; // apontar para a cópia persistente
...
#undef nvenc_status_case
  ...
  bool nvenc::nvenc_base::reconfigure_bitrate(uint32_t kbps) {
	std::scoped_lock lk(encoder_mutex);

	if (!encoder || !nvenc) {
		return false;
	}

	const uint32_t bps = kbps * 1000u;

	// Copias locais que vamos enviar para o reconfigure
	NV_ENC_INITIALIZE_PARAMS initCopy = init_params_snapshot; // snapshot atual
	NV_ENC_CONFIG           cfgCopy   = enc_config_snapshot;  // snapshot atual

	// Ajustar apenas o que é seguro em runtime (RC/bitrate); preserva rcMode existente
	cfgCopy.rcParams.averageBitRate = bps;
	cfgCopy.rcParams.maxBitRate     = bps;

	// (Opcional) sincronizar VBV se você usa CBR e driver/GPU suportam custom VBV.
	// Se você preferir não tocar no VBV, remova este bloco.
	if (cfgCopy.rcParams.vbvBufferSize == 0) {
		// Heurística conservadora: 1 segundo de buffer em bytes
		cfgCopy.rcParams.vbvBufferSize = bps;
	}
	if (cfgCopy.rcParams.vbvInitialDelay == 0) {
		cfgCopy.rcParams.vbvInitialDelay = cfgCopy.rcParams.vbvBufferSize / 2;
	}

	// Reamarrar ponteiro para a cópia local
	initCopy.encodeConfig = &cfgCopy;

	// Preparar estrutura de reconfigure com a "versão mínima" apropriada ao codec
	NV_ENC_RECONFIGURE_PARAMS rp = { min_struct_version(NV_ENC_RECONFIGURE_PARAMS_VER) };
	rp.reInitEncodeParams = initCopy;
	rp.resetEncoder = 0;   // não resetar o encoder (sem reinit do display)
	rp.forceIDR     = 1;   // boa prática ao trocar RC/bitrate

	NVENCSTATUS st = nvenc->nvEncReconfigureEncoder(encoder, &rp);
	if (st == NV_ENC_SUCCESS) {
		// Atualiza snapshots para refletir o estado atual pós-reconfigure
		enc_config_snapshot   = cfgCopy;
		init_params_snapshot  = initCopy;
		BOOST_LOG(info) << "NvEnc: bitrate reconfigured to " << kbps << " kbps";
		return true;
	}

	// Se quiser logar o erro de forma padronizada
	last_nvenc_error_string.clear();
	last_nvenc_error_string += "nvEncReconfigureEncoder failed";
	BOOST_LOG(warning) << "NvEnc: reconfigure failed (" << st << ")";
	return false;
  }

	bool nvenc::nvenc_base::reconfigure_framerate(uint32_t num, uint32_t den) {
		std::scoped_lock lk(encoder_mutex);
		if (!encoder || !nvenc) return false;

		// Snapshots atuais
		NV_ENC_INITIALIZE_PARAMS initCopy = init_params_snapshot;
		NV_ENC_CONFIG            cfgCopy  = enc_config_snapshot;

		// Ajuste do frame rate (apenas isso; mantém codec/profile/etc.)
		initCopy.frameRateNum = num;
		initCopy.frameRateDen = den;
		initCopy.encodeConfig = &cfgCopy;

		NV_ENC_RECONFIGURE_PARAMS rp = { min_struct_version(NV_ENC_RECONFIGURE_PARAMS_VER) };
		rp.reInitEncodeParams = initCopy;
		rp.resetEncoder = 0;   // não resetar o encoder
		rp.forceIDR     = 1;   // boa prática ao mudar timing

		NVENCSTATUS st = nvenc->nvEncReconfigureEncoder(encoder, &rp);
		if (st == NV_ENC_SUCCESS) {
			// Persistir novos snapshots
			init_params_snapshot = initCopy;
			enc_config_snapshot  = cfgCopy;
			BOOST_LOG(info) << "NvEnc: framerate reconfigured to "
							<< num << "/" << den;
			return true;
		}

		BOOST_LOG(warning) << "NvEnc: framerate reconfigure failed (" << st << ")";
		return false;
	}
