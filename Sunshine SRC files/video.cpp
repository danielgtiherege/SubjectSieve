...
namespace video {
  ...
  namespace {
	...
	struct RuntimeCfg {
		...
		std::optional<double> fps; // alvo do pacing
		std::optional<double> cap_fps;    // novo: limite superior
		std::optional<std::string> fps_cap_mode; // "drop" (default) ou "sleep"
	};
	
	static std::optional<RuntimeCfg> parse_runtime_json(const std::string& text) {
		RuntimeCfg out;
		...
		auto parse_int = [](std::string s) -> std::optional<int> {
			...
			// apenas dígitos + sinal
			if (!std::all_of(s.begin() + (s[0]=='-'||s[0]=='+'), s.end(), ::isdigit)) return std::nullopt;
			try {
		...
		auto parse_double = [](std::string s) -> std::optional<double> {
			// remove espaços e aspas; aceita vírgula como decimal
			s.erase(std::remove_if(s.begin(), s.end(), [](unsigned char c){
				return std::isspace(c) || c=='"';
			}), s.end());
		if (s.empty()) return std::nullopt;
		std::replace(s.begin(), s.end(), ',', '.'); // pt-BR friendly
		char* endp = nullptr;
		const double v = std::strtod(s.c_str(), &endp);
		if (endp == s.c_str() || *endp != '\0' || !std::isfinite(v) || v < 0.0) return std::nullopt;
		return v;
		};

		std::smatch m;
		// captura só o valor (grupo 1); tolera espaços e vírgula decimal
		std::regex re_bitrate(R"REGEX("bitrate(?:_kbps)?"\s*:\s*([0-9]+))REGEX", std::regex::icase);
		std::regex re_fps    (R"REGEX("(?:fps|framerate)"\s*:\s*([-+]?[0-9]+(?:[.,][0-9]+)?))REGEX", std::regex::icase);

		std::regex re_capfps(R"REGEX("(?:cap_fps|max_fps|limit_fps|framerate_cap)"\s*:\s*([-+]?[0-9]+(?:[.,][0-9]+)?))REGEX", std::regex::icase);
		std::regex re_mode  (R"REGEX("(?:fps_cap_mode)"\s*:\s*"([^"]+)")REGEX", std::regex::icase);

		if (std::regex_search(text, m, re_bitrate)) {
			if (auto v = parse_int(m[1].str())) out.bitrate_kbps = *v;
		}
		if (std::regex_search(text, m, re_fps)) {
			if (auto v = parse_double(m[1].str())) out.fps = *v;
		}

		if (std::regex_search(text, m, re_capfps)) {
			if (auto v = parse_double(m[1].str())) out.cap_fps = *v;
		}
		if (std::regex_search(text, m, re_mode)) {
			// aceita "drop" ou "sleep" (case-insensitive)
			auto s = m[1].str();
		...
		if (!out.bitrate_kbps && !out.fps) return std::nullopt; // nada válido
		return out;
	}
	...
	struct RuntimeWatcher {
		...
		// retorna config nova quando houver mudança; std::nullopt se nada mudou; {} (valor) se mudou mas não tinha campos válidos
		/*
		std::optional<RuntimeCfg> poll() {
			std::error_code ec;
			if (!std::filesystem::exists(path, ec)) return std::nullopt;
			auto wt = std::filesystem::last_write_time(path, ec);
			if (ec) return std::nullopt;
			if (wt == last_write) return std::nullopt;

			last_write = wt;
			std::ifstream f(path, std::ios::binary);
			if (!f) return RuntimeCfg{}; // mudou mas não abriu
			std::string s((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
			if (auto parsed = parse_runtime_json(s)) return parsed;
			return RuntimeCfg{}; // mudou mas sem campos reconhecidos
		}
		*/
		std::optional<RuntimeCfg> poll() {
			std::error_code ec;
			...
			std::ifstream f(path, std::ios::binary);
			if (!f) {
				BOOST_LOG(warning) << "[runtime] Não foi possível abrir " << path.string();
				return std::nullopt;
			}
			std::string s((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());

			// Debounce: se o arquivo está vazio/truncado, ignore
			if (s.size() < 3) return std::nullopt;

			if (auto parsed = parse_runtime_json(s)) {
				return parsed; // ok
			} else {
				BOOST_LOG(warning) << "[runtime] encoder_runtime.json inválido (sem campos numéricos válidos)";
				return std::nullopt; // <-- não sinaliza mudança
			}
		}
  ...
  public:
	...
	//Mudança de bitrate no encoder
	void set_bitrate_kbps(int kbps) {
		if (!avcodec_ctx) return;
		int64_t bps = static_cast<int64_t>(kbps) * 1000;
		avcodec_ctx->bit_rate   = bps;
		avcodec_ctx->rc_max_rate = bps;
		avcodec_ctx->rc_min_rate = bps;
		// Tenta repassar aos priv-opts (quando o encoder permitir)
		if (avcodec_ctx->priv_data) {
			av_opt_set_int(avcodec_ctx->priv_data, "b",       bps, 0);
			av_opt_set_int(avcodec_ctx->priv_data, "maxrate", bps, 0);
			av_opt_set_int(avcodec_ctx->priv_data, "minrate", bps, 0);
		}
	}
  ...
  public:
	...
	//Muda o FPS caso seja NVENC
	void set_bitrate_kbps(int kbps) {
		if (!device || !device->nvenc) return;
		device->nvenc->reconfigure_bitrate(static_cast<uint32_t>(kbps));
	}

	void set_output_framerate(uint32_t num, uint32_t den) {
		if (!device || !device->nvenc) return;
		device->nvenc->reconfigure_framerate(num, den);
	}
  ...
  void encode_run(
	...
	double runtime_min_fps_target = (config::video.minimum_fps_target > 0.0)
                                  ? config::video.minimum_fps_target
                                  : config.framerate;
	std::chrono::duration<double, std::milli> max_frametime {1000.0 / runtime_min_fps_target};
	BOOST_LOG(info) << "Minimum FPS target set to ~"sv
                << (runtime_min_fps_target / 2) << "fps ("sv
                << max_frametime.count() * 2 << "ms)"sv;	
				
				
	//cap de FPS. || 0 é sem cap
	using clock_t = std::chrono::steady_clock;
	std::chrono::duration<double, std::milli> min_frametime_cap {0.0};  // 0 => sem cap
	bool fps_cap_sleep = false;                                         // false=drop (default)
	std::optional<clock_t::time_point> last_sent_tp;                    // último frame enviado
				
	RuntimeWatcher rt;
	rt.path = std::filesystem::current_path() / "encoder_runtime.json";

	// helper para mudar bitrate de sessão conforme o tipo
	auto apply_bitrate_kbps = [&](int kbps) {
		if (auto *av = dynamic_cast<avcodec_encode_session_t*>(session.get())) {
			av->set_bitrate_kbps(kbps);
			BOOST_LOG(info) << "[runtime] AVCodec bitrate -> " << kbps << " kbps";
		} else if (auto *nv = dynamic_cast<nvenc_encode_session_t*>(session.get())) {
			nv->set_bitrate_kbps(kbps);
			BOOST_LOG(info) << "[runtime] NVENC bitrate request -> " << kbps << " kbps";
		}
	};

	// aplicar bitrate inicial do config para garantir que RC está em sincronia
	apply_bitrate_kbps(config.bitrate);			
    ...
    while (true) {
      ...
      if (shutdown_event->peek() || !images->running() || (reinit_event.peek() && frame_nr > 1)) {
		...
		if (now - last_poll >= 100ms) {
			last_poll = now;
			if (auto changed = rt.poll()) {
			// arquivo mudou; mesmo que vazio, é uma mudança
				
				if (changed->fps) {
					double new_fps = std::clamp(*changed->fps, 1.0, 480.0);
					runtime_min_fps_target = new_fps;
					max_frametime = std::chrono::duration<double, std::milli>{1000.0 / runtime_min_fps_target};
					BOOST_LOG(info) << "[runtime] pacing FPS -> " << runtime_min_fps_target
									<< " (max_frametime " << max_frametime.count() << " ms)";
				}
				if (changed->bitrate_kbps) {
					int kbps = std::clamp(*changed->bitrate_kbps, 100, 200000); // 100 kbps .. 200 Mbps
					apply_bitrate_kbps(kbps);
					session->request_idr_frame();
					BOOST_LOG(info) << "[runtime] bitrate -> " << kbps << " kbps (IDR solicitado)";
				}
				
				
				if (changed->cap_fps) {
					
					double cap_d = *changed->cap_fps;

					if (cap_d < 1.0) {
						// DESLIGAR CAP
						min_frametime_cap = std::chrono::duration<double, std::milli>{0.0};
						last_sent_tp.reset();
						BOOST_LOG(info) << "[runtime] FPS cap -> desativado";

						// (opcional) devolver framerate do encoder para o alvo original do config
						if (auto* nv = dynamic_cast<nvenc_encode_session_t*>(session.get())) {
							uint32_t fr = static_cast<uint32_t>(std::max(1.0, std::floor(config.framerate)));
							nv->set_output_framerate(fr, 1u);
						}
					} else {
						double cap = std::clamp(cap_d, 1.0, 480.0);
      ...
      if (!requested_idr_frame || images->peek()) {
		...
		/*
		Realiza o cap de FPS aqui e logo após o encode em si
		*/

		if (min_frametime_cap.count() > 0.0) {
			auto now = clock_t::now();
			if (!last_sent_tp) {
				// inicializa de modo a permitir envio imediato do primeiro frame
				//last_sent_tp = now - min_frametime_cap;
				last_sent_tp = now - std::chrono::duration_cast<clock_t::duration>(min_frametime_cap);
			}
			auto elapsed = now - *last_sent_tp;

			// Não “segure” IDR: deixe passar mesmo acima do cap (para confiabilidade)
			if (!requested_idr_frame && elapsed < min_frametime_cap) {
				auto remain = min_frametime_cap - elapsed;
				
				if (fps_cap_sleep) {
					// Modo "sleep": espera o restante do intervalo
					// (isso aumenta latência, mas força FPS pontual)
					if (remain > std::chrono::milliseconds(0)) {
						std::this_thread::sleep_for(std::chrono::duration_cast<std::chrono::milliseconds>(remain));
					}
				} else {
					// Modo "drop": descarta este frame para manter latência baixa
					// Evita busy-loop com um cochilo mínimo
					std::this_thread::sleep_for(std::chrono::milliseconds(1));
					continue; // volta ao topo do loop, sem chamar encode(...)
				}
			}
		}
      ...
      if (encode(frame_nr++, *session, packets, channel_data, frame_timestamp)) {
		...
		//Atualizamos o relógio antes de continuar
		last_sent_tp = clock_t::now();
