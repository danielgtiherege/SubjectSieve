    ...
    if (res > 0) {
      ...
      switch (event.type) {
        case ENET_EVENT_TYPE_RECEIVE:
          {
			//Intruzir aqui o log do evento enet
			ENetPeer* peer = event.peer;
			const size_t bytes = event.packet ? event.packet->dataLength : 0;
			const uint8_t ch   = event.channelID;

			// ENet já mantém RTT estimado por peer
			uint32_t rtt_ms = peer ? peer->roundTripTime : 0;
			uint32_t rtt_var= peer ? peer->roundTripTimeVariance : 0;

			// Muitos forks do ENet expõem packetLoss (%). Se NÃO existir no seu enet,
			// deixe como -1 para ficar vazio no CSV.
			int loss_pct = -1;
			#ifdef ENET_PEER_HAS_PACKET_LOSS
				loss_pct = (int)peer->packetLoss; // 0..100
			#endif

			metrics::log_input_enet_sample(rtt_ms, rtt_var, loss_pct, ch, bytes);
